...
def parse_sam_output(
    ...
    with pysam.FastaFile(topological_fasta_file_name) as fasta:
        for read in samfile.fetch():
            ...
            # For downstream PAM, we're working with revcomp'd sequences
            # So we need to extract PAM from the opposite end
            if pam_direction == "downstream":
                spacer = (
                    full_sequence[len(pam) :]
                    if len(full_sequence) > len(pam)
                    else full_sequence
                )
                extracted_pam = (
                    full_sequence[: len(pam)]
            ...
            else:  # upstream PAM stays the same
                spacer = (
                    full_sequence[len(pam) :]
                    if len(full_sequence) > len(pam)
                    else full_sequence
            ...
            if read.is_mapped:
                try:
                    target = read.get_reference_sequence()

                    # Handle target sequence RevComp and PAM trimming
                    if read.is_reverse:
                        target = str(Seq(target).reverse_complement())

                    # Always handle PAM in reference sequence before any other processing
                    target = target[len(pam) :]  # Always trim PAM from start
...
def run_bowtie_and_parse(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(os.devnull, "w") as devnull:
            ...
            console.log(
                f"Executing bowtie-build command: {' '.join(bowtie_build_command)}"
            )
            ...
            with tempfile.NamedTemporaryFile(delete=False) as bowtie_output_temp_file:
                bowtie_command = [
                    "bowtie",
                    "-S",  # output SAM
                    "--nomaqround",  # don't do rounding
                    "-q",  # input is fastq
                    "-a",  # report each non-specific hit
                    "--best",  # judge the *closest* non-specific match
                    "--tryhard",  # judge the *closest* non-specific match
                    "--chunkmbs",
                    "256*4",  # memory setting for --best flag
                    "-p",
                    str(num_threads),  # how many processors to use
                    "-n",
                    str(num_mismatches),  # allowable mismatches in seed
                    "-l",
                    "15",  # size of seed
                    "-e",
                    "39",  # dissimilarity threshold
                    "-m",
                    "100",  # keep only 1 alignment
                    "-x",
                    index_prefix,  # index base
                    sgrna_fastq_file_name,  # input fastq
                ]

                console.log(f"Executing bowtie command: {' '.join(bowtie_command)}")
