...
class MismatchDesignerGUI(QWidget):
    ...
    def initUI(self):
        ...
        # Replace existing save controls with interactive save button
    ...
    def browse_file(self):
        ...
        if dialog.exec_():
            ...
            if chosen:
                ...
                # Removed reference to self.output_file.setText()
    ...
    def save_mismatches(self):
        ...
        fname, _ = QFileDialog.getSaveFileName(
            self,
            "Save Data",
            os.path.join(os.path.dirname(os.path.dirname(__file__)), "mismatches.tsv"),
            "TSV Files (*.tsv)",
        )
        if not fname:
            return
        ...
        try:
            self.generated_mismatches.to_csv(fname, sep="\t", index=False)
